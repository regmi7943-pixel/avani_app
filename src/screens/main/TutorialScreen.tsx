          ...
          <View style={[styles.modalScreen, { backgroundColor: colors.background }]}>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 50 }} showsVerticalScrollIndicator={false}>
              ...
              {isParsingSubtitles || !parsedDetails ? (
                ...
                <>
                  {/* Detailed Summary Section parsed by Grok AI */}
                  <View style={{ paddingHorizontal: 18, marginTop: 16 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                      <Ionicons name="sparkles" size={16} color={colors.brandGreen} />
                      <Text style={[styles.subHeadingTitle, { color: colors.text, marginBottom: 0 }]}>
                        {language === 'ne' ? 'Grok AI भिडियो उपशीर्षक सारांश' : 'Grok AI Video Summary'}
                      </Text>
                    </View>
                    <Text style={{ fontSize: 13, color: colors.text, lineHeight: 20 }}>
                      {language === 'ne' ? parsedDetails.summaryNe : parsedDetails.summaryEn}
                    </Text>
                    {parsedDetails.reasoning ? (
                      <View style={{ marginTop: 8, padding: 10, backgroundColor: isDarkMode ? '#1A2E22' : '#E8F5E9', borderRadius: 8, borderWidth: 1, borderColor: colors.brandGreen }}>
                        <Text style={{ fontSize: 11.5, color: colors.brandGreen, fontWeight: '700' }}>
                          💡 {parsedDetails.reasoning}
                        </Text>
                      </View>
                    ) : null}
                  ...
                  {parsedDetails.specializedSection?.contentEn ? (
                    <View style={{ paddingHorizontal: 18, marginTop: 18 }}>
                      <Text style={[styles.subHeadingTitle, { color: colors.text }]}>
                        📌 {language === 'ne' ? parsedDetails.specializedSection.titleNe : parsedDetails.specializedSection.titleEn}
                      </Text>
                      <View style={[styles.dosageTableCard, { backgroundColor: colors.card, borderColor: colors.border, padding: 14 }]}>
                        <Text style={{ fontSize: 12.5, color: colors.text, lineHeight: 19 }}>
                          {language === 'ne' ? parsedDetails.specializedSection.contentNe : parsedDetails.specializedSection.contentEn}
                        </Text>
                      </View>
                    </View>
                  ) : null}

                  {/* Exact Agronomic Dosage & Schedule Table (Only for Crop Videos) */}
                  {parsedDetails.dosageTable?.basalEn && !parsedDetails.dosageTable.basalEn.toLowerCase().includes('not applicable') && !parsedDetails.dosageTable.basalEn.toLowerCase().includes('n/a') ? (
                    <View style={{ paddingHorizontal: 18, marginTop: 18 }}>
                      <Text style={[styles.subHeadingTitle, { color: colors.text }]}>
                        📊 {language === 'ne' ? 'बीउ, सिँचाइ र मलको वैज्ञानिक परिमाण' : 'Agronomic Dosage & Input Quantity'}
                      </Text>

                      <View style={[styles.dosageTableCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                        <View style={[styles.tableRow, { borderBottomColor: colors.border }]}>
                          <Text style={[styles.tableCellLabel, { color: colors.secondaryText }]}>
                            {language === 'ne' ? 'मापन एकाइ' : 'Unit Basis'}
                          </Text>
                          <Text style={[styles.tableCellValue, { color: colors.brandGreen, fontWeight: '800' }]}>
                            {parsedDetails.dosageTable.unit}
                          </Text>
                        </View>

                        <View style={[styles.tableRow, { borderBottomColor: colors.border }]}>
                          <Text style={[styles.tableCellLabel, { color: colors.secondaryText }]}>
                            {language === 'ne' ? 'रोप्ने बेला / आधार मल' : 'Basal / Sowing'}
                          </Text>
                          <Text style={[styles.tableCellValue, { color: colors.text }]}>
                            {language === 'ne' ? parsedDetails.dosageTable.basalNe : parsedDetails.dosageTable.basalEn}
                          </Text>
                        </View>

                        <View style={[styles.tableRow, { borderBottomColor: colors.border }]}>
                          <Text style={[styles.tableCellLabel, { color: colors.secondaryText }]}>
                            {language === 'ne' ? 'सिँचाइ / टप-ड्रेस' : 'Top Dress / Irrig'}
                          </Text>
                          <Text style={[styles.tableCellValue, { color: colors.text }]}>
                            {language === 'ne' ? parsedDetails.dosageTable.topDressNe : parsedDetails.dosageTable.topDressEn}
                          </Text>
                        </View>

                        <View style={styles.tableRow}>
                          <Text style={[styles.tableCellLabel, { color: colors.secondaryText }]}>
                            {language === 'ne' ? 'विषादी / सूक्ष्म तत्व' : 'Pesticide / Spray'}
                          </Text>
                          <Text style={[styles.tableCellValue, { color: colors.text }]}>
                            {language === 'ne' ? parsedDetails.dosageTable.sprayNe : parsedDetails.dosageTable.sprayEn}
                          </Text>
                        </View>
                      </View>
                    </View>
                  ...
                  {/* Step-by-Step Practical Operating Manual */}
                  {parsedDetails.stepsEn && parsedDetails.stepsEn.length > 0 ? (
                    <View style={{ paddingHorizontal: 18, marginTop: 20 }}>
                      <Text style={[styles.subHeadingTitle, { color: colors.text }]}>
                        📝 {language === 'ne' ? 'चरणबद्ध मुख्य कार्यहरू (Practical Action Steps)' : 'Step-by-Step Practical Action Plan'}
                      </Text>

                      <View style={{ gap: 12, marginTop: 12 }}>
                        {(language === 'ne' ? parsedDetails.stepsNe : parsedDetails.stepsEn).map((step, idx) => (
                          <View 
                            key={idx} 
                            style={[styles.stepItemRow, { backgroundColor: colors.card, borderColor: colors.border }]}
                          >
                            <View style={[styles.stepNumberBadge, { backgroundColor: colors.brandGreen }]}>
                              <Text style={{ color: '#fff', fontSize: 12, fontWeight: '800' }}>{idx + 1}</Text>
                            </View>
                            <Text style={[styles.stepTextContent, { color: colors.text }]}>
                              {step}
                            </Text>
                          </View>
                        ))}
                  ...
                  {parsedDetails.precautionsEn && parsedDetails.precautionsEn.length > 0 ? (
                    <View style={{ paddingHorizontal: 18, marginTop: 22 }}>
                      <View style={[styles.takeawaysCard, { backgroundColor: isDarkMode ? '#2b1e1e' : '#fff3f3', borderColor: '#E53935' }]}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                          <Ionicons name="warning-outline" size={20} color="#E53935" />
                          <Text style={{ fontSize: 14, fontWeight: '800', color: '#E53935' }}>
                            ⚠️ {language === 'ne' ? 'विशेष सावधानी तथा सुरक्षा नियमहरू' : 'Safety & Agronomic Precautions'}
                          </Text>
                        </View>

                        {(language === 'ne' ? parsedDetails.precautionsNe : parsedDetails.precautionsEn).map((point, idx) => (
                          <View key={idx} style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 8, marginTop: 6 }}>
